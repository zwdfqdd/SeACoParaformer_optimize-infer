...
模型（扁平存于 settings.PUNC_MODEL_DIR，默认 models/punc）：
    model_quant.onnx（量化，~280MB）/ tokens.json（272727，CharTokenizer 逐字）；
    运行时仅需这两个文件（punc_list 已按 config.yaml 探测结果硬编码于 _PUNC_LIST，
    不在运行时解析 config.yaml）。缺失时加载阶段自动调 scripts/download_punc.py（HTTP 直链）下载。

ONNX 契约（已探测，punc_list 源自模型 config.yaml，固化于 _PUNC_LIST）：
